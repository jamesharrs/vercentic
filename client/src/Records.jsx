...
export const PANEL_META = {
  ...
  share:        { icon:"share",         label:"Share & Promote",     defaultOpen:false, jobOnly:true },
...
export const RecordDetail = ({ record, fields, allObjects, environment, objectName, objectColor, onClose, fullPage, onToggleFullPage, onUpdate, onDelete, onNavigate, featureFlags = {} }) => {
  ...
  const panelVisible = (id) => {
    ...
    // Feature flag gates for sharing and fraud panels
    if (id === 'share') return canRecord('record_share_job');
    if (id === 'fraud') return canRecord('record_fraud_analysis');
  ...
  const renderPanel = useCallback(({ id }) => {
    ...
    return null;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [previewAtt, record, editing, notes, attachments, fields, environment, objectName, fileTypes, cvParsing, cvParseAtt, docExtracting, docExtractAtt, uploading, uploadDragging, selectedFileType, currentObject, allObjects, openPanels, _permCtx, uploadError, globalEdit, saving, panelSections]);
