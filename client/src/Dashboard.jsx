...
export default function Dashboard({ environment, session, onNavigate, onOpenRecord, onReport, onViewAll }) {
  ...
  const goTo = (slug, key, val) => window.dispatchEvent(new CustomEvent("vercentic:filter-navigate", { detail: { objectSlug: slug, fieldKey: key, fieldValue: val } }));
  const openRpt = (cfg) => { if (onReport) onReport(cfg); else window.dispatchEvent(new CustomEvent("vercentic:open-report", { detail: cfg })); };
    ...
    <div style={{ flex: 1, padding: "28px 32px 80px 32px", fontFamily: "'DM Sans',-apple-system,sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <ActionBtn label="Add Candidate" color={V.purple}
          ...
          onClick={() => window.dispatchEvent(new CustomEvent("vercentic:create-record", { detail: { objectName: "People" } }))} />
        <ActionBtn label="Post a Job" color={V.rose}
          ...
          onClick={() => window.dispatchEvent(new CustomEvent("vercentic:create-record", { detail: { objectName: "Jobs" } }))} />
