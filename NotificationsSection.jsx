// NotificationsSection.jsx — Notification preferences for Settings
// Renders inside Settings.jsx as a section under "Your preferences"
// Expects: api helper available from parent, or uses fetch directly
...
const C = {
  ...
  green: '#0CA678', purple: '#7C3AED', amber: '#F59F00', red: '#E03131',
  blue: '#3B5BDB',
};
...
const Ic = ({ n, s = 16, c = "currentColor" }) => (
  <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c}
    ...
    <path d={ICON_PATHS[n] || ICON_PATHS.bell} />
  </svg>
);
...
const TIMEZONES = [
  'Asia/Dubai', 'Asia/Riyadh', 'Asia/Qatar', 'Asia/Singapore',
...
export default function NotificationsSection() {
  ...
  const [digestConfig, setDigestConfig] = useState({
    daily_time: '08:00', weekly_day: 'friday', timezone: 'Asia/Dubai',
  });
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch('/api/notification-preferences', {
        headers: {
          'X-Tenant-Slug': localStorage.getItem('vrc_tenant') || '',
          'X-User-Id': localStorage.getItem('vrc_user_id') || '',
        },
      });
  ...
  const togglePref = (key, channel) => {
    setTypes(prev => prev.map(t =>
      t.key === key ? { ...t, [channel]: !t[channel] } : t
    ));
  };

  const toggleAllInCategory = (catKey, channel, value) => {
    setTypes(prev => prev.map(t =>
      t.category === catKey ? { ...t, [channel]: value } : t
    ));
  };
  ...
  const save = async () => {
    ...
    const preferences = {};
    types.forEach(t => {
      preferences[t.key] = { in_app: t.in_app, email: t.email };
    });
    try {
  ...
  return (
    <div style={{ fontFamily: F, color: C.text1, maxWidth: 760 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: C.text3, lineHeight: 1.5 }}>
            Choose which notifications you receive, how they're delivered, and when.
          </p>
        </div>
        <button
          onClick={save}
      ...
      <div style={{
        ...
        border: `1.5px solid ${quietHours.enabled ? '#334155' : C.border}`,
        transition: 'all .2s',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: quietHours.enabled ? 14 : 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: quietHours.enabled ? '#f1f5f9' : C.text1 }}>
                Quiet hours
              </div>
              <div style={{ fontSize: 12, color: quietHours.enabled ? '#94a3b8' : C.text3 }}>
                Pause non-urgent notifications during off hours
              </div>
            </div>
          </div>
        ...
        {quietHours.enabled && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <input
                type="time" value={quietHours.end}
            ...
            <span style={{ fontSize: 11, color: '#64748b', marginLeft: 'auto' }}>
              Urgent alerts (agent reviews, workflow blocks) still come through
            </span>
          </div>
        )}
      </div>
        ...
        return (
          <div key={cat.key} style={{
            ...
            <button
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: C.text1, marginBottom: 1 }}>
                  {cat.label}
                </div>
                <div style={{ fontSize: 11, color: C.text3 }}>{cat.description}</div>
              </div>
              <span style={{
                fontSize: 11, fontWeight: 600, color: enabledCount > 0 ? cat.color : C.text3,
                padding: '2px 8px', borderRadius: 99,
                background: enabledCount > 0 ? `${cat.color}12` : 'transparent',
              }}>
            ...
            {isExpanded && (
              <div style={{ padding: '0 18px 16px' }}>
                ...
                <div style={{
                  ...
                  padding: '8px 0 10px', borderBottom: `1px solid ${C.border}`,
                  marginBottom: 6,
                }}>
                  <span style={{ flex: 1 }} />
                  {!isDigests && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, width: 80, justifyContent: 'center' }}
                      title="Toggle all in-app notifications for this category">
                      ...
                      <button onClick={() => toggleAllInCategory(cat.key, 'in_app', !allInApp)}
                        style={{
                          width: 14, height: 14, borderRadius: 3, border: `1.5px solid ${allInApp ? C.accent : C.border}`,
                          background: allInApp ? C.accent : 'transparent', cursor: 'pointer',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, width: 80, justifyContent: 'center' }}
                    title="Toggle all email notifications for this category">
                    ...
                    <button onClick={() => toggleAllInCategory(cat.key, 'email', !allEmail)}
                      style={{
                        width: 14, height: 14, borderRadius: 3, border: `1.5px solid ${allEmail ? C.accent : C.border}`,
                        background: allEmail ? C.accent : 'transparent', cursor: 'pointer',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0,
                      }}>
                ...
                {catTypes.map(nt => (
                  <div key={nt.key} style={{
                    display: 'flex', alignItems: 'center', gap: 8,
                    padding: '10px 0', borderBottom: `1px solid ${C.border}08`,
                  }}>
                    <div style={{ flex: 1 }}>
                ...
                {/* AI Match threshold — only for AI category */}
                {cat.key === 'ai' && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <span style={{
                        fontSize: 16, fontWeight: 800, color: C.purple,
                        background: `${C.purple}12`, padding: '3px 10px', borderRadius: 8,
                      }}>
                        {matchThreshold}%
                      </span>
                    </div>
                ...
                {/* Digest scheduling config — only for digests category */}
                {isDigests && (
                  <div style={{
                    ...
                    <div style={{ fontSize: 12, fontWeight: 700, color: C.text1, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <Ic n="clock" s={13} c={C.red} />
                      Delivery schedule
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, color: C.text3, display: 'block', marginBottom: 4 }}>
                          Daily digest time
                        </label>
                        <input
                          type="time" value={digestConfig.daily_time}
                      ...
                      <div>
                        ...
                        <select
                          value={digestConfig.weekly_day}
                          onChange={e => setDigestConfig(d => ({ ...d, weekly_day: e.target.value }))}
                          style={{
                            width: '100%', padding: '7px 10px', borderRadius: 8,
                            border: `1px solid ${C.border}`, fontSize: 12, fontFamily: F,
                            color: C.text1, background: 'white', textTransform: 'capitalize',
                          }}>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={e => setDigestConfig(d => ({ ...d, timezone: e.target.value }))}
                          style={{
                            width: '100%', padding: '7px 10px', borderRadius: 8,
                            border: `1px solid ${C.border}`, fontSize: 12, fontFamily: F,
                    ...
                    <div style={{ marginTop: 10, fontSize: 11, color: C.text3, lineHeight: 1.5 }}>
                      ...
                      Weekly summary sends every {digestConfig.weekly_day.charAt(0).toUpperCase() + digestConfig.weekly_day.slice(1)} afternoon.
                      Monthly report sends on the 1st of each month.
                    </div>
                  </div>
                )}
              </div>
      ...
      <div style={{
        ...
        background: C.bg, fontSize: 12, color: C.text3, lineHeight: 1.6,
