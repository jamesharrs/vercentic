// server/routes/environments.js  — with response caching
...
// ── LIST — cached 2 minutes (environments almost never change) ────────────────
...
router.get('/:id', (req, res) => {
  const e = findOne('environments', x=>x.id===req.params.id);
...
router.patch('/:id', (req, res) => {
  const e = update('environments', x=>x.id===req.params.id, req.body);
